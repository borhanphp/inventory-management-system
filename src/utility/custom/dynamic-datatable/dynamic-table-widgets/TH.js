...
const TH = ( props ) => {
    const { column, index: i, getLeftDistanceOfFixedColumn, expandableRows, handleSort, setAllColumns, allColumns, tableId, columnCache } = props;
    ...
    return <th
        ...
        style={{
            ...
            left: column.isFixed && getLeftDistanceOfFixedColumn( expandableRows ? i + 1 : i )
        }}
        id={theadId}
    >
        <p style={{
            ...
            margin: 0,
            width: '100%',
            justifyContent: column.type === 'action' || column.center ? 'center' : 'flex-start'
        }}>
            <span
                onClick={() => handleSort( column )}
                style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}
            >
                {column.name} {column.sortable && <ChevronIcon
                    size={16}
                />
                }
            </span>
