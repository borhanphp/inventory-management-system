...
import { bindItemTypeInfo, getAllItemTypeByFilter, getItemTypeById } from '../store';
...
const ItemTypeList = () => {
  const { allData, totalItems } = useSelector( ( { itemtypes } ) => itemtypes );
  ...
  const handleEdit = ( row ) => {
    ...
    dispatch( getItemTypeById( row?.id ) );
  };
  ...
  const getItems = ( page, perPage ) => {
    ...
    dispatch( getAllItemTypeByFilter( submittedData ) )
  }
