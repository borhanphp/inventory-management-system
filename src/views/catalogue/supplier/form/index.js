...
import { addNewSupplier, bindSupplierInfo, getSupplierByFilter } from '../store';
...
const SupplierForm = () => {
  ...
  const formSchema = yup.object().shape( {
    ...
    contactType: supplierBasicInfo?.contactType ? yup.string() : yup.string().required( 'contactType is Required!!!' ),
  ...
  useEffect( () => {
    return () => {
      dispatch( bindSupplierInfo( null ) )
    }
  }, [] )
  ...
  const handleOnSubmit = () => {
    ...
    dispatch( addNewSupplier( submittedData ) )
      .then( ( res ) => {
        if ( res.error ) {
          return;
        }
        ...
        <FormContentLayout title="Basic Information">
          <Col xs={12} sm={12} md={6} lg={4} xl={3} xxl={3}>
            <ErpSelect
              ...
              classNames={classNames( `mt-1 erp-dropdown-select ${( errors && errors?.contactType && !supplierBasicInfo?.contactType ) && 'is-invalid'}` )}
