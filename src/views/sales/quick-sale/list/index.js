...
import SaleReturn from '../form/SaleReturn';
import { bindInstantSaleData, getInstantSaleByFilter, getInstantSaleById } from '../store';
...
const InstantPurchaseList = () => {
    const { allInstantSale, loading, totalItems } = useSelector( ( { instantSale } ) => instantSale );
    ...
    const [openInstantSaleModal, setOpenInstantSaleModal] = useState( false );


    const handleReturn = ( row ) => {
        handleInstantSaleModalOpen();
        dispatch( getInstantSaleById( row?.id ) )
            .then( ( res ) => {
                const updatedInfo = {
                    ...res?.payload,
                ...
                dispatch( bindInstantSaleData( updatedInfo ) )
            } )

    };

    const handleDetails = ( row ) => {
        navigate( '/instant-sales-details', { state: row.id } )
        dispatch( getInstantSaleById( row?.id ) );
    ...
    const handleInstantSaleModalOpen = () => {
        setOpenInstantSaleModal( true );
    };

    const handleInstantSaleModalClosed = () => {
        setOpenInstantSaleModal( false );
    };
    ...
    const getItems = ( page, perPage ) => {
        ...
        dispatch( getInstantSaleByFilter( submittedData ) )
    }
    ...
    return (
        <>
            ...
            {
                openInstantSaleModal && <SaleReturn
                    openInstantSaleModal={openInstantSaleModal}
                    handleInstantSaleModalClosed={handleInstantSaleModalClosed}
                    instantSaleToggle={() => setOpenInstantSaleModal( !openInstantSaleModal )}
                />
            }
