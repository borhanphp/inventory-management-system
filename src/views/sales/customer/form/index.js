...
import { addNewCustomer, bindCustomerInfo, getCustomerByFilter } from '../store';
...
const CustomerForm = () => {
  ...
  const formSchema = yup.object().shape( {
    ...
    contactType: customerBasicInfo?.contactType ? yup.string() : yup.string().required( 'contactType is Required!!!' ),
  ...
  const handleOnSubmit = () => {
    ...
    dispatch( addNewCustomer( submittedData ) )
      .then( ( res ) => {
        if ( res.error ) {
          return;
        }
        ...
        <FormContentLayout title="Basic Information">
          <Col xs={12} sm={12} md={6} lg={4} xl={3} xxl={3}>
            <ErpSelect
              ...
              classNames={classNames( `mt-1 erp-dropdown-select ${( errors && errors?.contactType && !customerBasicInfo?.contactType ) && 'is-invalid'}` )}
