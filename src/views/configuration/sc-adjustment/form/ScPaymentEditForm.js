...
import { bindScAdjustmentInfo, getAllScAdjustmentByFilter, updateScAdjustment } from "../store";
import { initialScAdjustmentData } from "../store/model";

const ScPaymentEditForm = ( { editFormOpen, toggleEditSidebar, currentPage } ) => {
  ...
  console.log( 'amount', scAdjustmentBasicInfo?.amount )

  const handleOnSubmit = () => {
    const { id, scId, headId, amount, currencyId, note, currentCurrencyRate, avgCurrencyRate, date, dueAmount } = scAdjustmentBasicInfo;
    const finalAmount = ( currentCurrencyRate - avgCurrencyRate ) * dueAmount;
    ...
    const submittedData = {
      ...
      amount: amount ? +amount : finalAmount?.toFixed( 4 ),
      note: note,
    ...
    dispatch( updateScAdjustment( submittedData ) )
      .then( ( res ) => {
        if ( res.error ) {
          return;
        } else {
          ...
          toggleEditSidebar()
          toast.success( 'SC Adjustment Updated' )
        }
      ...
      >
        <div>
          ...
          <ErpInput
            ...
            type="number"
            classNames="mt-1"
            // value={scAdjustmentBasicInfo?.currentCurrencyRate ? ( scAdjustmentBasicInfo?.currentCurrencyRate - scAdjustmentBasicInfo?.avgCurrencyRate ) * scAdjustmentBasicInfo?.dueAmount : scAdjustmentBasicInfo?.amount}
...
export default ScPaymentEditForm;
