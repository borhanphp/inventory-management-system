...
import { bindScAdjustmentInfo, getAllScAdjustmentByFilter, getScAdjustmentById } from '../store';
...
const index = () => {
  const { allScAdjustments, totalItems, loading } = useSelector( ( { scAdjustments } ) => scAdjustments );
  ...
  const handleEdit = ( row ) => {
    ...
    dispatch( getScAdjustmentById( row?.id ) );
  };
  ...
  const getItems = ( page, perPage ) => {
    ...
    dispatch( getAllScAdjustmentByFilter( submittedData ) )
  }
