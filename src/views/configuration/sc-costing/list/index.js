...
import { bindScCostingInfo, getAllScCostingByFilter, getScCostingById } from '../store';
...
const index = () => {
  const { allData, totalItems, loading } = useSelector( ( { scCostings } ) => scCostings );
  ...
  const handleEdit = ( row ) => {
    ...
    dispatch( getScCostingById( row?.id, handleOpenEditSidebar ) );
  };
  ...
  const toggleSidebar = () => {
    dispatch( bindScCostingInfo( initialScCostingData ) )
  ...
  const getItems = ( page, perPage ) => {
    ...
    dispatch( getAllScCostingByFilter( submittedData ) )
  }
