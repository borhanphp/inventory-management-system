...
import CurrencyEditForm from '../form/CurrencyEditForm';
import { bindCurrencyInfo, getAllCurrencyByFilter, getCurrencyById } from '../store';
import { initialCurrencyData } from '../store/model';
import { currencyColumn } from './columns';
...
const index = () => {
  const { allData, totalItems, loading } = useSelector( ( { currencies } ) => currencies );
  ...
  const handleEdit = ( row ) => {
    ...
    dispatch( getCurrencyById( row?.id ) );
  };
  ...
  const toggleSidebar = () => {
    dispatch( bindCurrencyInfo( initialCurrencyData ) )
  ...
  const getItems = ( page, perPage ) => {
    ...
    dispatch( getAllCurrencyByFilter( submittedData ) )
  }
        ...
        <Card className='overflow-hidden'>
          ...
          <div className='px-2'>
            ...
            <DataTable
              ...
              // expandableRows={true}
              // expandOnRowClicked
              columns={currencyColumn( handleEdit )}
