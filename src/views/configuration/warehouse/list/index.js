...
import WarehouseEditForm from "../form/WarehouseEditForm";
import { bindWarehouseInfo, getAllWarehouseByFilter, getWarehouseById } from "../store";
...
const WarehouseList = () => {
  const { allData, totalItems, loading } = useSelector( ( { warehouses } ) => warehouses );
  ...
  const handleEdit = ( row ) => {
    toggleEditSidebar();
    dispatch( getWarehouseById( row?.id ) );
  };
  ...
  const getItems = ( page, perPage ) => {
    ...
    dispatch( getAllWarehouseByFilter( submittedData ) )
  }
  ...
  return (
    <>
        ...
        <Card className="overflow-hidden">
          ...
          <div className='px-2'>
            ...
            <DataTable
              ...
              // expandableRows={true}
              // expandOnRowClicked
              columns={warehouseColumn( handleEdit )}
      ...
      {editFormOpen && (
        <WarehouseEditForm currentPage={currentPage} editFormOpen={editFormOpen} toggleEditSidebar={toggleEditSidebar} />
      )}
    </>
  );
};
