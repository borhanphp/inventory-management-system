...
import { addNewPi, bindPiInfo } from "../store";
...
const PiForm = () => {
    ...
    const scItems = scData?.items?.filter( ( item ) => {
        return !piDataItems.some( ( piItem ) => piItem.id === item.id );
    } );

    const scFromList = location.state
    ...
    useEffect( () => {
        if ( scFromList ) {
            dispatch( getScById( scFromList.value ) )
                .then( ( res ) => {
                    const { buyerId, sellerBankId, currencyId, currencyRate, insurance, lastDateOfShipment } = res.payload;
                    const updatedObj = {
                        ...piData,
                        scId: scFromList,
                        buyerId: buyerId,
                        ...
                        insurance: insurance,
                        latestShipmentDate: lastDateOfShipment,
                        groups: []
                    };
    ...
    const handleOnSubmit = ( cb ) => {
        const { scId, buyerId, sellerBankId, currencyId, piCode, piDate, from, to, latestShipmentDate, shipment, insurance, payment, note, currencyRate } = piData;
        ...
        dispatch( addNewPi( storeData ) )
            .then( ( res ) => {
                ...
                if ( res.error ) {
                    ...
                    toast.success( cb === "publish" ? 'PI Created Successfully' : 'PI Saved as Draft' )
                }
    ...
    const handleGroupDelete = ( id ) => {
        const filteredData = piData?.groups?.filter( d => d.id !== id )
                                                ...
                                                piData?.groups?.map( ( row, i ) => (
                                                    <Fragment key={i}>
                                                        <tr>
                                                            ...
                                                            <GroupTd> {( row.quantity * row.price ).toFixed( 2 )}</GroupTd>
                                                        </tr>
                                                                                ...
                                                                                {row?.items?.map( ( item, ii ) => {
                                                                                    return (
                                                                                        <tr key={ii}>
                                                                                            ...
                                                                                            <ChildTd>{item?.description}</ChildTd>
