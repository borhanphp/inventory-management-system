...
import { addNewLocalPurchase, bindLocalPurchaseInfo } from "../store";
...
const PurchaseForm = () => {
    ...
    const handleOnSubmit = () => {
        const { poId, supplierInvoiceCode, supplierInvoiceDate, supplierId, invoiceType, totalAmount, items } = basicLocalPurchaseData;
        const submittedData = {
            poId: 0,
            ...
            items: items.map( ( item ) => ( {
                poItemId: item?.itemId ? item?.itemId : 0,
                itemId: item?.id,
        ...
        dispatch( addNewLocalPurchase( submittedData ) )
            .then( ( res ) => {
                if ( res.error ) {
                    return;
                } else {
