...
import { bindPurchaseInfo, getPurchaseByFilter, getPurchaseById } from "../../purchase/store";
import { bindScInfo, getScById, updateSc } from "../store";
...
const EditScLcForm = () => {
    ...
    useEffect( () => {
        dispatch( getScById( scId ) )
            .then( ( response ) => {
                dispatch( getPurchaseById( response.payload.poId.value ) )
                    .then( ( res ) => {
                        dispatch( bindPurchaseInfo( {
                            ...res,
                            items: res.payload.items.filter( dd => !dd.isUsedInSC )
                        } ) )
                    } )
            } )
        return () => {
    ...
    const handleModalSubmit = () => {
        const newItems = selectedItems.filter(
            selectedItem => !scData.items.some( existingItem => existingItem.id === selectedItem.id )
        );
        dispatch( bindScInfo( {
            ...scData,
            items: [...scData.items, ...newItems?.map( d => ( { ...d, uoMId: { label: d.uoM, value: d.uoMId } } ) )],
    ...
    const handleOnSubmit = ( cb ) => {
        ...
        const submittedData = {
            ...
            totalAmount: discountedAmount ? ( subTotalAmount - discountedAmount ) : totalAmount,
        ...
        dispatch( updateSc( submittedData ) )
            .then( ( res ) => {
                console.log( res )
                if ( res.error ) {
                    return;
                } else {
                    navigate( '/sc-details', { state: scId } );
                    dispatch( bindScInfo( initialScData ) )
                    toast.success( cb === "publish" ? 'SC Updated Successfully' : 'SC Saved as Draft' )
                }
                    ...
                    <div >
                        <FormContentLayout title="SC Information">
                            ...
                            <Col md={3} xl={3} xxl={3}>
                                <InInput
                                    // classNames="mt-1"
                                    label="Destination Loading Port"
                            ...
                            <Col md={3} xl={3} xxl={3}>
                                <InInput
                                    ...
                                    // value={scData?.discountedAmount ? ( scData.subTotalAmount - scData.discountedAmount ).toFixed( 2 ) : scData.totalAmount.toFixed( 2 )}
                                    onChange={( e ) => { handleInputOnChange( e ) }}
                                />
                            </Col>
