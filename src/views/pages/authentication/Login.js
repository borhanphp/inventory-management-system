  ...
  return (
    <div className='d-flex'>
      ...
      <div className='d-flex flex-column'>
        ...
        <span>You have successfully logged in as an {role}.</span>
      </div>
    </div>
  );
};
...
const Login = () => {
  ...
  const onSubmit = data => {
    if ( Object.values( data ).every( field => field.length > 0 ) ) {
      axios.post( 'http://192.168.0.9:81/api/v1/identity/login', {
        userNameOrEmail: data.loginEmail,
        ...
        .then( res => {
          ...
          toast( t => (
            <ToastContent t={t} role={userData.role || 'admin'} name={userData.firstName + userData.lastName || userData.username} />
          ) );
        } )
        .catch( err => {
          console.log( 'err', err )
